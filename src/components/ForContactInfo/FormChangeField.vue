<template>
  ...
  <form class="form-to-change" :class="{active__form: selectField.onChange}" @submit.prevent="onChange">
    <h1>Изменение поля: <span>{{selectField.nameField}}</span></h1>
    <div class="inputs">
      <div>
        <label @click="onLabel">Имя поля:</label>
        <input @focus="onInput" @blur="notFocus" v-model="dataChangeField.nameField" type="text">
      </div>
      <div>
        <label @click="onLabel">Значение поля:</label>
        <input @focus="onInput" @blur="notFocus" v-model="dataChangeField.valueField" type="text">
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    onChange() {
      ...
      if (this.dataChangeField.name !== null && this.dataChangeField.valueField !== null) {
