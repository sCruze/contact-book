...
export default {
    ...
    mutations: {
        ...
        createPreviousStates(state, id) {
            const searchContact = Object.assign({}, state.contacts.find(item => item.id === id))
            ...
            if (state.contactStatusChanges.find(item => item.id === searchContact.id) === undefined) {
                ...
                const findContact = Object.assign({}, newPreviousStates.find(item => item.id === searchContact.id))
            ...
            } else if (state.contactStatusChanges.find(item => item.id === searchContact.id)) {
                // Создаем переменную с найденным по id объектом
                const findContact = state.contactStatusChanges.find(item => item.id === searchContact.id)
        ...
        removeContact(state, id) {
            ...
            localStorage.removeItem('contacts')
            localStorage.setItem('contacts', JSON.stringify(state.contacts.filter(item => item.id !== id)))
            state.contacts = JSON.parse(localStorage.getItem('contacts'))
        ...
        stepBack(state, id) {
            ...
            if (state.contactStatusChanges.length > 0) {
                ...
                let contactStatusChanges = Object.assign([], state.contactStatusChanges)
                // Находим нужный объект по id
                const allPreviousStatesOfThisContact = contactStatusChanges.find(item => item.id === id)
                ...
                if (allPreviousStatesOfThisContact.previousStates.length !== 0) {
                    ...
                    for (let key in stateContacts) {
                        ...
                        if (stateContacts[key].id === id) {
                            ...
                            const allPreviousStatesThisContact = state.contactStatusChanges.find(item => item.id === id)
                            // Перезаписываем объект по совпадающему ключу, перезаписываем последним объектом из
                            // массива предыдущих состояний
                            stateContacts[key] = allPreviousStatesThisContact
                                .previousStates[allPreviousStatesThisContact.previousStates.length - 1]
